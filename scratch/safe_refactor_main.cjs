...
if (mainTs.includes(oldFunc)) {
	...
	// Let's print out what we see so we can fix it if it fails
	console.error("COULD NOT FIND applyGraphicsQuality exact string");
}
...
mainTs = mainTs.replace(/GraphicsQuality/g, 'QualityLevel');
