...
code = code.replace(/this\.volumetricFogPass = new VolumetricFogPass\(\);[\s\S]*?this\.volumetricFogPass\.setQuality\(this\.graphicsQuality\);/, setupNew);
...
code = code.replace(/if \(this\.volumetricFogPass\) this\.volumetricFogPass\.enabled = false;/g, '');
