...
// 6. Resize
const resizeOld = `		this.volumetricFogPass?.setSize(
			window.innerWidth * pr,
			window.innerHeight * pr
		);`;
const resizeNew = `		this.composer?.setSize(window.innerWidth, window.innerHeight);`;
code = code.replace(resizeOld, resizeNew);
const resizeOld2 = `		this.volumetricFogPass?.setSize(
			window.innerWidth * pr,
			window.innerHeight * pr
		);`;
code = code.replace(resizeOld2, resizeNew); // setAspectResolution
code = code.replace('this.resizePondTargets();\n\t\tconst pr = this.renderer.getPixelRatio();\n\t\tthis.composer?.setSize(window.innerWidth, window.innerHeight);\n\t\tthis.syncVolumetricFogQuality();', 'this.resizePondTargets();\n\t\tthis.composer?.setSize(window.innerWidth, window.innerHeight);\n\t\tthis.syncVolumetricFogQuality();'); // Clean up unused pr
