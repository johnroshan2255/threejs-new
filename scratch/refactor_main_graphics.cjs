...
// 7. General replacements of remaining \`graphicsQuality\` checks to \`resolutionQuality\` (which serves as a proxy for misc stuff like fog updates)
mainTs = mainTs.replace(/this\.graphicsQuality/g, 'this.resolutionQuality');

fs.writeFileSync('src/main.ts', mainTs);
console.log("Refactored main.ts graphics");
